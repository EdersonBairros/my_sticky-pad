...
| **3.12.2** | 🟢 PATCH | Renomeia a extensão de "My sticky pad" para **"Sticky Pad"** no `manifest.json` (campo `name`, exibido na barra de ferramentas, página de extensões e título da listagem nas lojas) e no título do `README.md` — alinhando com o nome usado em toda a documentação, changelog e materiais (Forms, ícone). Sem mudança de comportamento |
| **3.12.1** | 🟢 PATCH | Atualiza o `description` do `manifest.json` (resumo curto usado pela Chrome Web Store/Firefox AMO na listagem, puxado automaticamente do manifest): de um texto genérico para um resumo com os principais recursos (lembretes, categorias, cores, emojis), reforçando que os dados ficam 100% locais. Sem mudança de comportamento na extensão |
| **3.12.0** | 🟡 MINOR | **Reportar um bug**: novo item no menu ⋮ ("🐛 Reportar um bug") que abre, numa aba nova, um Google Forms dedicado (descrição do problema, passos para reproduzir, gravidade, navegador, anexo de print/vídeo). O campo "Navegador usado" é pré-selecionado automaticamente (detecção via `navigator.userAgent`/`navigator.brave`, cobrindo Chrome/Firefox/Brave/Vivaldi/Edge). Optou-se por abrir o Forms de verdade em vez de `mailto:` ou envio silencioso via `fetch`: só a tela real do Google suporta anexar arquivo (exige login do respondente, mas isso já seria necessário de qualquer forma) e uma aba `https://` normal não dispara o aviso nativo de "abrir app externo?" que quebra visualmente num popup pequeno (visto com o `mailto:` nesta mesma versão, revertido antes de chegar ao usuário) |
| **3.11.7** | 🟢 PATCH | Corrige o bloqueio de imagens (3.11.6) para o caso de colar **texto com imagem embutida junto** (de uma página, Word, etc.): `bindEditorPasteGuard()` só detectava imagem "pura" vinda como arquivo no clipboard (screenshot, "copiar imagem"), e deixava passar quando a imagem vinha embutida como `<img>` dentro do HTML colado ao lado de texto — a imagem entrava normalmente e só sumia ao salvar, sem aviso. Agora também detecta esse caso (regex no `text/html` do clipboard), bloqueando a imagem e colando só o texto, com o mesmo toast de aviso |
| **3.11.6** | 🟢 PATCH | Bloqueia imagens coladas no editor (Ctrl+V de screenshot, "copiar imagem", etc.). Sem isto, o navegador inseria a imagem de verdade no editor — só sumia depois, ao salvar, pelo `sanitizeHtml()` (choke point de XSS), dando a impressão de que "sumiu sozinha" sem explicação. Novo `bindEditorPasteGuard()` (`src/ui/editor.js`) intercepta o `paste` só quando a área de transferência tem um **arquivo de imagem**: bloqueia a imagem e mostra um toast, colando o texto (se houver) normalmente. Colar texto puro ou formatado **não muda em nada** — só age quando detecta imagem. Cobre nota nova e edição de nota existente (mesmo `.note-editor`, sem código duplicado) |
| **3.11.5** | 🟢 PATCH | Corrige a importação de notas no **Firefox** com a extensão fixada na barra de ferramentas: o Firefox fecha popups de extensão ao perder o foco, e abrir o seletor de arquivo do sistema operacional conta como perda de foco — cancelando a escolha do arquivo antes do evento `change` disparar (falha silenciosa: nada acontecia, sem erro). Na janela flutuante isso não ocorre (janela de verdade, não popup ancorado). Agora o clique em "Importar" detecta o Firefox (via `navigator.userAgent`, não mais `typeof browser` — builds recentes de Chrome também expõem esse namespace) fora da janela flutuante e mostra um aviso claro pedindo para abrir em janela flutuante, em vez da falha silenciosa de antes |
| **3.11.4** | 🟢 PATCH | Ajustes a partir do `web-ext lint` (validador oficial da Mozilla, 0 erros/12 avisos → 0 erros/11 avisos, os restantes revisados e confirmados como falsos positivos — conteúdo já passa por `escapeHtml`/`sanitizeHtml` ou é dado interno/estático). Corrige `browser_specific_settings.gecko.strict_min_version` de `140.0` para `142.0` (era incompatível com `data_collection_permissions` no Firefox Android, suportado só a partir da 142). Reforça `_htmlToText` (usado na busca) para extrair texto via `<template>` inerte em vez de `<div>` "vivo" — mesma técnica do choke point de `sanitizeHtml()`, elimina o risco teórico de handlers (`onerror`/`onload`) disparando ao processar HTML antigo/malformado |
| **3.11.3** | 🟢 PATCH | Preparação para publicar nas lojas (Chrome Web Store, Firefox AMO — Brave usa o mesmo pacote do Chrome). Adicionado `src/utils/browser-polyfill.js` (unifica `browser`→`chrome` no Firefox, que expõe os dois namespaces simultaneamente; sem isso `await chrome.storage.local.get()` poderia retornar `undefined` lá), carregado como primeiro script em `popup.html`. Sem mudança de comportamento no Chrome/Brave. Adicionado `tools/package.ps1` (gera `dist/sticky-pad-v<versão>.zip` só com os arquivos de runtime, pronto para envio) e `dist/` ao `.gitignore` |
